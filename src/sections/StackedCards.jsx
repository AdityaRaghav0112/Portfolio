...
const StackedCards = () => {
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen w-full flex items-center justify-center mt-6 md:mt-10">
        <h1 className="absolute top-0 font-anton text-black 
                       text-3xl sm:text-4xl md:text-5xl">
          Featured Work
        </h1>

        <div className="relative w-[95%] md:w-[90%] h-[85%] md:h-[80%] rounded-3xl overflow-hidden">
          {cards.map((card, i) => (
            <div
              key={i}
            ...
            >
              <div
                className="min-h-full w-full rounded-4xl flex text-white overflow-hidden
                           max-lg:w-[95%] max-md:flex-col-reverse"
              >
                {/* TEXT */}
                <div
                  className="border-r-4 border-black w-[55%] 
                             p-6 md:p-8 flex flex-col justify-between
                             max-md:w-full max-md:border-r-0 max-md:border-b-4"
                >
                  <div>
                    <h1
                ...
                {/* IMAGE */}
                <div
                  className="w-[45%] min-h-[60vh] relative overflow-hidden
                             max-md:w-full max-md:min-h-[35vh]"
                >
                  <img
                    src={card.img}
                    alt="Project preview"
                    className="absolute inset-0 w-full h-full 
                               object-cover object-right select-none"
                  />
                </div>
              </div>
            </div>
